...
-(void)inittips
{
    ...
    self.contentSize = CGSizeMake(self.frame.size.width, lb_tips.frame.origin.y+60);
}
...
#pragma -mark- 上拉下拉刷新的方法
- (void)refreshViewBeginRefreshing:(MJRefreshBaseView *)refreshView
{
    [webscro._header endRefreshing];
    CGPoint point = self.contentOffset;
    [UIView animateWithDuration: 0.25 animations: ^{
        self.contentOffset = CGPointMake(0, point.y-webscro.frame.size.height);
        self.contentSize = CGSizeMake(self.frame.size.width, bt_shop.frame.origin.y+bt_shop.frame.size.height+100);
